...
| RP2040 | XIP_SSI `SR.TFE/TFNF/RFNE/BUSY` (offset 0x28) + `DR0` (offset 0x60), driving a W25Q flash responder (RDSR1 0x05 / RDSR2 0x35 / WREN 0x06 / WRSR 0x01) | Datasheet §4.10 (DW_apb_ssi): boot2 configures the SSI, then read-status/set-quad-enable/poll-WIP over the QSPI flash, then arms XIP. §2.8.3 boot2. Winbond W25Q080 §8 status registers (WIP=SR1.0, WEL=SR1.1, QE=SR2.1). | `Rp2040XipSsi` (`crates/core/src/peripherals/rp2040/xip_ssi.rs`): a real SSI shift engine — each `DR0` write clocks one 8-bit frame out and one back into the RX FIFO; `SR` reports live FIFO state (TFE/TFNF always set, BUSY never, RFNE tracks RX). The attached flash answers RDSR/WREN/WRSR from modeled SR1/SR2 state. One frame per `DR0` write (CTRLR0.DFS_32 not honoured); program/erase complete instantly so SR1.WIP always reads clear. The XIP window (0x10000000) is the directly-mapped flash image. | Modeled 2026-07-04, **pending HW validation**. Fixes the boot2/XIP bus-read fault at 0x18000028: the stage-2 bootloader and the pico-sdk `flash_enable_xip_via_boot2` RAM copy now complete their SSI setup + status polling. |
| RP2040 | SIO `SPINLOCK0..31` (offsets 0x100..0x17c) | Datasheet §2.3.1.5: reading a free spinlock atomically claims it and returns a nonzero value; reading a held lock returns 0; writing releases it. | `Rp2040Sio` (`crates/core/src/peripherals/rp2040/sio.rs`): single-core try-lock — a free lock is granted on read (returns `1<<n`), a held lock reads 0, a write clears the held bit. Unblocks the pico-sdk `hw_claim_lock`/`spin_lock_blocking` spin the mutex/alarm-pool init relies on. | Modeled 2026-07-04, **pending HW validation**. |
| nRF52840 | Legacy UART `TXD` (0x51C) → `EVENTS_TXDRDY` (0x11C) | PS rev 1.7 §6.34: UART0/UARTE0 share one MMIO window; ENABLE=4 selects the legacy UART, ENABLE=8 selects UARTE. In legacy mode, writing a byte to `TXD` transmits it and hardware raises `EVENTS_TXDRDY` once the shifter is free for the next byte (delay = one character time at the configured BAUDRATE, after `TASKS_STARTTX` armed the transmitter). | `Nrf52Uarte` (`crates/core/src/peripherals/nrf52/uarte.rs`): a `TXD` write emits the byte to the UART sink and sets `EVENTS_TXDRDY` on the same access. `TASKS_STARTTX` no longer arms EasyDMA in legacy mode (ENABLE=4) — that path is UARTE-only. Byte is sent instantly (no baud-rate delay), and the transmitter-armed precondition is not enforced. | Modeled 2026-07-04, **pending HW validation**. Unblocks the Adafruit/Arduino nRF52 core, whose `Uart::write` does `TXD=byte; while(EVENTS_TXDRDY==0); EVENTS_TXDRDY=0` and previously spun forever (regression: `test_nrf52840_arduino_serial_survival`). |
