...
The `version` input defaults to `v0.24.0` and independently selects the
immutable Core CLI release archive named
`labwired-v0.24.0-<platform>.tar.gz`. The action downloads that public archive
from the `w1ne/labwired-core` GitHub release with `curl`; it does not build Core
on the consumer runner.

Its inputs are exactly `script` (required), `version` (default `v0.24.0`),
