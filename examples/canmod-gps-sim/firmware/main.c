...
{
    /* AFIOEN and IOPAEN are new: without a clocked GPIOA the PA9 mux in
     * uart_init() is dropped on the gated bus and the pad stays dark. */
    RCC_APB2ENR |= RCC_APB2ENR_AFIOEN | RCC_APB2ENR_IOPAEN | RCC_APB2ENR_USART1EN;
    RCC_APB1ENR |= RCC_APB1ENR_CAN1EN;
}

/* --- GPIOA (F1 layout: CRL @ 0x00, CRH @ 0x04). The F1 pad mux is four bits
 * per pin — MODE[1:0] then CNF[1:0]. There is no MODER and no AFR on this
 * family, so there is no AF number to write: a pin's alternate function is
 * fixed by the pin. --- */
 ...
 * i.e. 115942 baud, 0.6% fast and well inside a UART's budget. */
 ...
 * analyzer on PA9 reads the GPIO output latch — a flat line — while the
