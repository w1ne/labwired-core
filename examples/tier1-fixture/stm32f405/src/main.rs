...
//! STM32F405 Tier-1 fixture firmware (Cortex-M4F, thumbv7em-none-eabi).
//! The F405 is the F407 without Ethernet, CAN2 or the second/third I2C and
//! SPI instances, and its console UART is `uart2` in the descriptor where the
//! F407 calls the same 0x4000_4400 block `usart2`. Every peripheral this
//! fixture drives is declared by `configs/chips/stm32f405.yaml` at the address
//! used below — checked against the descriptor, not assumed from the family.
//!
//! Validates the simulator's chip model peripheral-by-peripheral with RAW
//! REGISTER accesses against the peripherals wired by
//! `configs/chips/stm32f405.yaml`, reporting one line per peripheral class
...
//!
//! `dma` and `irq` are NOT reported: the F405 yaml declares no DMA/NVIC-class
...
// ── Wired peripherals (configs/chips/stm32f405.yaml) ──────────────────────
...
fn check_i2c() -> Result<(), &'static [u8]> {
    // clock-gating: I2C1 (RCC_APB1ENR.I2C1EN bit 21, ENR @ 0x40) is unclocked
    // out of reset. A pre-clock CR1.PE write is dropped and reads back 0; enable
