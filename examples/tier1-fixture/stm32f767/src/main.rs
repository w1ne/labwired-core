...
//! STM32F767 Tier-1 fixture firmware (Cortex-M7, thumbv7em-none-eabi).
//! The F767 is a Cortex-M7 part with 2 MiB of flash and 512 KiB of SRAM, and
//! six more GPIO ports and an I2C4 that the F4 family does not have. The
//! peripherals this fixture drives are the ones `configs/chips/stm32f767.yaml`
//! declares, at the addresses used below; the F7 keeps the F4 addresses for
//! all of them, which is why the family image ran at all — and why a
//! part-specific fixture is the thing that actually proves this model.
//!
//! Validates the simulator's chip model peripheral-by-peripheral with RAW
//! REGISTER accesses against the peripherals wired by
//! `configs/chips/stm32f767.yaml`, reporting one line per peripheral class
...
//!
//! `dma` and `irq` are NOT reported: the F767 yaml declares no DMA/NVIC-class
...
// ── Wired peripherals (configs/chips/stm32f767.yaml) ──────────────────────
const RCC_BASE: u32 = 0x4002_3800; // type rcc, profile stm32f4
const GPIOA_BASE: u32 = 0x4002_0000; // type stm32f4_gpio → stm32v2 layout
const UART2_BASE: u32 = 0x4000_4400; // `uart2` in this descriptor; same 0x4000_4400 block
...
const CONSOLE: Console = Console::new(UART2_BASE, UART2_BASE + 0x04, 1 << 7);
...
fn check_timer() -> Result<(), &'static [u8]> {
    // clock-gating: TIM2 (RCC_APB1ENR.TIM2EN bit 0, ENR @ 0x40) is unclocked out
    // of reset, so its registers read 0 / drop writes. Prove the dead state —
    // ARR reads 0, not its 0xFFFFFFFF clocked reset — then enable the bus clock.
    if rd32(TIM2_BASE + 0x2C) != 0 {
        return Err(b"timer-gated");
    }
    wr32(RCC_BASE + 0x40, rd32(RCC_BASE + 0x40) | 0x1); // APB1ENR.TIM2EN
...
/// I2Cv2, not the classic F1/F4 map: `configs/chips/stm32f767.yaml` declares
/// `profile: "stm32l4"` for i2c1, so this is the L476 sequence — CR2.START,
/// ISR.BUSY, a NACKed one-byte write with AUTOEND — rather than SR1.SB, which
/// does not exist on this peripheral.
fn check_i2c() -> Result<(), &'static [u8]> {
    // Gating first, then the protocol. I2C1 is clocked by RCC_APB1ENR.I2C1EN
    // (bit 21, ENR @ 0x40) on this part: a pre-clock CR1.PE write is dropped
    // and reads back 0. The L476 fixture skips this because its descriptor
    // leaves i2c1 ungated; here the gate is real, so the check proves it
    // before touching the v2 register map behind it.
    wr32(I2C1_BASE, 1); // dropped while gated
    if rd32(I2C1_BASE) != 0 {
        return Err(b"i2c-gated");
    }
    wr32(RCC_BASE + 0x40, rd32(RCC_BASE + 0x40) | (1 << 21)); // APB1ENR.I2C1EN
...
fn check_spi() -> Result<(), &'static [u8]> {
    // clock-gating: SPI1 (RCC_APB2ENR.SPI1EN bit 12, ENR @ 0x44) is unclocked
    ...
    wr32(RCC_BASE + 0x44, rd32(RCC_BASE + 0x44) | (1 << 12)); // APB2ENR.SPI1EN
...
fn check_adc() -> Result<(), &'static [u8]> {
    // clock-gating: ADC1 (RCC_APB2ENR.ADC1EN bit 8, ENR @ 0x44) is unclocked out
    // of reset. A pre-clock CR2.ADON write is dropped and CR2 reads back 0;
    // enable the bus clock, then power up and convert.
    wr32(ADC1_BASE + 0x08, 1); // dropped while gated
    if rd32(ADC1_BASE + 0x08) != 0 {
        return Err(b"adc-gated");
    }
    wr32(RCC_BASE + 0x44, rd32(RCC_BASE + 0x44) | (1 << 8)); // APB2ENR.ADC1EN
