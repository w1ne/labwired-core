...
//! | timg0             | timg          | timer  | PASS — T0UPDATE latches an advancing counter |
...
//! | system/rtc_cntl   | system/clock  | clock  | not attempted → `unrecorded` |
//! | dma               | dma           | dma    | not attempted → `unrecorded` |
//!
//! `clock` and `dma` are declared by `esp32c3.yaml` (so they are NOT `na`) but
//! are wired as declarative register files with no behavioral engine. This
//! fixture deliberately makes no claim about them: `unrecorded`, not faked.
//!
...
// STATUS @ 0x1C (TXFIFO_CNT in bits [25:16]).
//
// The C3 now binds the real Espressif UART twin (EspUart, shared with the S3):
// a 128-entry hardware TX FIFO that shifts out at the baud rate and DROPS a
// byte written to a full FIFO, exactly as silicon does. The previous
// `uart0_write_byte` blasted bytes with no occupancy check — correct against
// the old vendor-neutral STM32F1 model (writes went straight to the sink), but
// against the twin it overran the FIFO and silently truncated the transcript
// mid-line, so `TIER1 done` never arrived and the whole row degraded to
// partial/blocked. Poll STATUS.TXFIFO_CNT for space first, like the S3 fixture.
...
//
// configs/chips/esp32c3.yaml wires timg0 as `esp32_timg` — a behavioral model
// with a live counter:
...
//
// Enable the counter, latch it twice with a spin gap, and require the second
// snapshot to be strictly greater. A register file cannot produce that.
...
// produce it).
//
// TIMING: the engine is bit-level and cycle-stretched — it drives real SDA/SCL
// waveforms at the programmed SCL period, so a command list "does NOT complete
// synchronously" with the `TRANS_START` write (i2c.rs module docs). The one
// thing that IS synchronous is `TRANS_START` auto-clearing. So poll INT_RAW for
// `TRANS_COMPLETE` in a bounded loop, exactly as driver firmware does; the
// original write-then-read-once check was written against the older
// complete-on-write engine and now reports a false `i2c-no-complete`.
...
fn check_ledc() -> Result<(), &'static str> {
    ...
    // Hold the timer in reset, clear stale state, then release it.
    //
    // DUTY_RES = 14 → period = 1<<14 = 16384 counts at divider 1. The period
    // must not alias with the peripheral tick quantum
    // (`bus::RECOMMENDED_TICK_INTERVAL` = 512 CPU cycles, what `labwired run`
    // uses): the legacy walk advances the counter in 512-count steps, so with
    // the old period of 1024 the counter was only ever observable as 0 or 512
    // and two reads inside the same quantum returned the identical value —
    // reported as a bogus `ledc-not-counting` even though the model was
    // counting and wrapping (LSTIMER0_OVF latched). 16384 gives 32 distinct
    // sample points per period and 32 quanta of headroom before the first wrap.
    reg_write(TIMER0_CONF, conf(14, 1) | RST);
    reg_write(INT_CLR, LSTIMER0_OVF);
    reg_write(TIMER0_CONF, conf(14, 1)); // release reset, start counting

    if reg_read(INT_RAW) & LSTIMER0_OVF != 0 {
        return Err("ledc-ovf-early"); // must not have wrapped yet
    }

    // (1) The live counter advances with elapsed cycles. Bounded poll rather
    // than a fixed spin so the check does not depend on the tick quantum.
    let a = reg_read(TIMER0_VALUE);
    let mut advanced = false;
    for _ in 0..200_000 {
        if reg_read(TIMER0_VALUE) != a {
            advanced = true;
            break;
        }
    }
    ...
    // (2) Run long enough to wrap the 16384-count period and latch overflow.
    ...
    reg_write(TIMER0_CONF, conf(14, 1) | PAUSE);
    reg_write(INT_CLR, LSTIMER0_OVF);
...
#[entry]
fn main() -> ! {
    // gpio, timer, irq, i2c, spi, adc, ledc — ordered by increasing complexity.
