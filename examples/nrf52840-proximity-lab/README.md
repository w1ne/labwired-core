...
and raises an **ALARM** output when the target is within a 150 mm threshold —
the same "set a target distance, watch the flag go high when in range" loop you
would build on the physical board.

The firmware talks only to raw nRF GPIO registers (no LabWired APIs), so the
**same ELF runs in the simulator and flashes to real silicon**.
...
| ECHO   | P1.05  | MCU input        | sensor holds it high ∝ distance        |
...
`IN_RANGE`, `LAST_TICKS` at `0x2000_0000…`), and the ALARM line is the nRF P0
`OUT` register (`0x5000_0504`, bit 6). The test scripts assert all of them:

| Scenario        | DISTANCE_MM | IN_RANGE | ALARM (P0.06) |
|-----------------|-------------|----------|---------------|
| Near (10 cm)    | 100         | 1        | high (0x40)   |
| Far  (30 cm)    | 299         | 0        | low  (0x00)   |
