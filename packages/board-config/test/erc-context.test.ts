...
/** Minimal nRF52840 diagram: generic mcu part, board key = 'nrf52840' (PIN_MAPS entry). */
const nrfDiagram = (over: Partial<DiagramV2> = {}): DiagramV2 => ({
  version: 2,
  board: 'nrf52840',
  parts: [{ id: 'mcu', type: 'mcu' }, { id: 'b1', type: 'bme280' }],
  nets: [],
  connections: [],
  wires: [],
  ...over,
...
describe('effectivePin — raw-first nRF52840 dotted pin names', () => {
  it('resolves mcu:P0.00 to bidirectional on nrf52840 board (raw name must not be stripped)', () => {
    // nRF52840 pins are literally named P0.00..P1.15 in PIN_MAPS.
    // Without raw-first lookup, /\.\d+$/ strips P0.00→P0→null and ERC silently skips.
    const ctx = buildContext(nrfDiagram());
    const pin = effectivePin(ctx, { part: 'mcu', pin: 'P0.00' });
    expect(pin).not.toBeNull();
    expect(pin!.etype).toBe('bidirectional');
  });

  it('resolves mcu:P1.10 to bidirectional on nrf52840 board', () => {
    const ctx = buildContext(nrfDiagram());
    const pin = effectivePin(ctx, { part: 'mcu', pin: 'P1.10' });
    expect(pin).not.toBeNull();
    expect(pin!.etype).toBe('bidirectional');
  });

  it('GND.2 suffix-strip still works on esp32-s3-zero (regression guard)', () => {
    // GND exists in esp32-s3-zero PIN_MAPS; GND.2 must strip to GND→power_out.
    const ctx = buildContext(minDiagram());
    const pin = effectivePin(ctx, { part: 'mcu', pin: 'GND.2' });
    expect(pin).not.toBeNull();
    expect(pin!.etype).toBe('power_out');
  ...
  it('resolves bme280 GND pin on nrf52840 board via catalog decl (raw lookup)', () => {
    // bme280 catalog has pin named GND (no suffix); raw lookup must find it.
    const ctx = buildContext(nrfDiagram());
    const pin = effectivePin(ctx, { part: 'b1', pin: 'GND' });
    expect(pin).not.toBeNull();
    expect(pin!.etype).toBe('power_in');
  });
});
