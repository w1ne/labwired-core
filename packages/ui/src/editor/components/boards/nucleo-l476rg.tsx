...
export const nucleoL476rgComponent: ComponentDef = {
  type: 'nucleo-l476rg',
  label: 'NUCLEO-L476RG',
  category: 'mcu',
  width: W,
  ...
  render: (_attrs, state) => {
    ...
    return (
      <g>
        ...
        <defs>
          <linearGradient id="nucl476-pcb" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0" stopColor="#f5f5f0" />
            <stop offset="1" stopColor="#e3e0d6" />
          </linearGradient>
          <linearGradient id="nucl476-stlink" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0" stopColor="#ecebe5" />
            <stop offset="1" stopColor="#d8d4c8" />
          </linearGradient>
          <linearGradient id="nucl476-chip" x1="0" y1="0" x2="0" y2="1">
            <stop offset="0" stopColor="#2a2a2a" />
            <stop offset="1" stopColor="#0e0e0e" />
          </linearGradient>
          <linearGradient id="nucl476-pad" x1="0" y1="0" x2="0" y2="1">
        ...
        <rect
          ...
          height={H}
          rx={6}
          fill="url(#nucl476-pcb)"
          stroke={selected ? '#F062B8' : '#5a5648'}
          strokeWidth={selected ? 3 : 1}
        />

        {/* ST-LINK area */}
        <rect x={4} y={4} width={W - 8} height={stLinkH} fill="url(#nucl476-stlink)" stroke="#bdb7a8" strokeWidth={0.5} />
        ...
        {/* ST-LINK MCU */}
        <rect x={28} y={26} width={32} height={32} rx={1} fill="url(#nucl476-chip)" stroke="#000" strokeWidth={0.5} />
        ...
        {/* Target MCU LQFP-64 */}
        <rect x={W / 2 - 44} y={H / 2 - 44} width={88} height={88} rx={2} fill="url(#nucl476-chip)" stroke="#000" strokeWidth={1} />
        ...
        {/* LD2 user LED on PA5 */}
        <circle cx={W - 60} cy={H - 100} r={4} fill="#222" stroke="#000" strokeWidth={0.4} />
        <circle cx={W - 60} cy={H - 100} r={2.4} fill="#3DD68C" opacity={0.85} />
        <text x={W - 60} y={H - 86} textAnchor="middle" fill="#444" fontFamily="'JetBrains Mono', monospace" fontSize={5.5}>LD2</text>

        {/* User button B1 (blue) */}
        <rect x={W - 60} y={H - 76} width={32} height={22} rx={2} fill="#1F5A82" stroke="#0E3552" strokeWidth={0.7} />
        <circle cx={W - 44} cy={H - 65} r={6} fill="#2A6F9C" stroke="#0E3552" strokeWidth={0.5} />
        <text x={W - 44} y={H - 44} textAnchor="middle" fill="#444" fontFamily="'JetBrains Mono', monospace" fontSize={5.5}>USER</text>

        {/* Reset button */}
        <rect x={20} y={H - 76} width={28} height={16} rx={2} fill="#202020" stroke="#000" strokeWidth={0.5} />
        <circle cx={34} cy={H - 68} r={5} fill="#555" stroke="#2a2a2a" strokeWidth={0.5} />
        <text x={34} y={H - 48} textAnchor="middle" fill="#444" fontFamily="'JetBrains Mono', monospace" fontSize={5.5}>NRST</text>
        ...
        <text
          ...
          fontSize={7}
        >
          MB1136 · STM32L4 Nucleo-64
        </text>

        {/* Morpho headers - GPIO pin pads left (PA) */}
        {allPins.filter((p) => p.side === 'left').map((p) => (
          <g key={p.id}>
            <rect x={-3} y={p.y - 4} width={12} height={8} fill="url(#nucl476-pad)" stroke="#7a5a1a" strokeWidth={0.3} />
        ...
        {/* Morpho headers - GPIO pin pads right (PB) */}
        {allPins.filter((p) => p.side === 'right').map((p) => (
          <g key={p.id}>
            <rect x={W - 9} y={p.y - 4} width={12} height={8} fill="url(#nucl476-pad)" stroke="#7a5a1a" strokeWidth={0.3} />
        ...
        {/* Top GPIO pin pads (PC) */}
        {allPins.filter((p) => p.side === 'top').map((p) => (
          <g key={p.id}>
            <rect x={p.x - 4} y={-3} width={8} height={12} fill="url(#nucl476-pad)" stroke="#7a5a1a" strokeWidth={0.3} />
            <circle cx={p.x} cy={3} r={1.5} fill="#0a0a0a" />
            <text x={p.x} y={18} textAnchor="middle" fill="#3a342a" fontFamily="'JetBrains Mono', monospace" fontSize={5.5}>
              {p.label}
            </text>
          </g>
        ))}
        ...
        <text x={4} y={12} fill="#7a7363" fontFamily="'JetBrains Mono', monospace" fontSize={6.5} fontWeight={600}>
          CN5
        </text>
