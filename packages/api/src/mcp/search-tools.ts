...
export const SEARCH_TOOLS_TOOL: McpTool = {
  ...
  description:
    'Search the LabWired MCP tool catalog by keyword and return relevant tool definitions. Use this to find board discovery, component discovery, diagram validation, firmware run, Playground session, and hardware diagnosis tools.',
  inputSchema: {
    type: 'object',
    required: ['query'],
    properties: {
...
function toolTokens(tool: McpTool): string[] {
...
export function rankTools(query: string, tools: readonly McpTool[], limit = 8): RankedTool[] {
...
function toRanked(tools: readonly McpTool[]): RankedTool[] {
