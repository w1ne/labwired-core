...
5. Compile the diagram to a board manifest with `labwired_compile_diagram`. The hosted connector returns the manifest inline as `system_yaml` (no filesystem persistence on the server). Pass it directly to `labwired_run` as the `system` argument. Warnings are included in the response; errors abort compilation.
6. Compile firmware outside hosted MCP using the documented scaffold and target flags.
7. Run the ELF with `labwired_run`.
