// Guards the tab-visibility logic of the mobile bottom sheet: BLE + Serial are
// always offered (parity with the desktop always-on Air Tracer), while Inputs,
// Logic, and IO-Link appear only when the diagram contains a matching part.
import { describe, it, expect, vi } from 'vitest';
import { render, screen } from '@testing-library/react';
import type { Diagram, Part } from '@labwired/ui';
...
function renderSheet(d: Diagram) {
  return render(
    <MobileInputsSheet
      diagram={d}
      boardIoStates={{}}
...
describe('MobileInputsSheet tabs', () => {
  it('always offers Serial and BLE, even for a diagram with no special parts', () => {
    renderSheet(diagram([part({ type: 'led' })]));
    const tabs = tabNames();
    expect(tabs).toContain('Serial');
    expect(tabs).toContain('BLE');
