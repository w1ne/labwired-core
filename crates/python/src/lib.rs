...
use labwired_core::{
    Arch, DebugControl, StopReason, SimulationError,
...
impl From<StopReason> for PyStopReason {
    fn from(r: StopReason) -> Self {
        match r {
            StopReason::Breakpoint(pc) => PyStopReason { kind: "breakpoint".to_string(), pc: Some(pc) },
            StopReason::StepDone => PyStopReason { kind: "step_done".to_string(), pc: None },
            StopReason::MaxStepsReached => PyStopReason { kind: "max_steps_reached".to_string(), pc: None },
            StopReason::ManualStop => PyStopReason { kind: "manual_stop".to_string(), pc: None },
        }
    }
}
...
fn build_bus(system_path: Option<PathBuf>) -> anyhow::Result<SystemBus> {
    ...
    let chip_path = sys_path.parent().unwrap_or(&PathBuf::from(".")).join(&manifest.chip);
...
impl Machine {
    ...
    fn new(firmware: String, system_config: Option<String>) -> PyResult<Self> {
        ...
        let machine: Box<dyn DebugControl + Send> = match program.arch {
            ...
            _ => return Err(pyo3::exceptions::PyValueError::new_err("Unsupported architecture")),
        };
    ...
    fn step(&mut self, max_steps: Option<u32>) -> PyResult<PyStopReason> {
        ...
        let reason = guard.run(max_steps)
    ...
    fn read_memory(&self, addr: u32, len: usize) -> PyResult<Vec<u8>> {
        let guard = self.inner.lock().unwrap();
        guard.read_memory(addr, len).map_err(PySimulationError).map_err(Into::into)
    }
    ...
    fn write_memory(&mut self, addr: u32, data: Vec<u8>) -> PyResult<()> {
        let mut guard = self.inner.lock().unwrap();
        guard.write_memory(addr, &data).map_err(PySimulationError).map_err(Into::into)
    }
    ...
    fn snapshot(&self) -> PyResult<String> {
        ...
        serde_json::to_string(&snap).map_err(|e| pyo3::exceptions::PyRuntimeError::new_err(e.to_string()))
    }
    ...
    fn restore(&mut self, json_snapshot: String) -> PyResult<()> {
        let mut guard = self.inner.lock().unwrap();
        let snap: labwired_core::snapshot::MachineSnapshot = serde_json::from_str(&json_snapshot)
             .map_err(|e| pyo3::exceptions::PyValueError::new_err(e.to_string()))?;
        guard.restore(&snap).map_err(PySimulationError).map_err(Into::into)
    }
