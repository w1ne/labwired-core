...
fn test_h5_demo_uart_output() {
    ...
    if !system_path.exists() {
        ...
        println!("System config not found at {:?}, checking relative to Cargo.toml", system_path);
    }
    ...
    let chip = ChipDescriptor::from_file(&chip_path).expect(&format!("Failed to load chip descriptor at {:?}", chip_path));
    ...
    machine.load_firmware(&program).expect("Failed to load firmware");
