...
/// Build an ESP32-C3 ROM-boot machine: faithful BROM reset + flash image +
/// the peripheral wiring the real boot needs. `efuse_mac`, when set, programs
/// a distinct factory MAC so multiple instances are distinguishable on the
/// shared [`virtual_wifi`] air. Extracted so the single-device run and the
/// two-device WiFi run share one construction.
fn build_c3_rom_boot_machine(
    ...
    if let Some(mac) = efuse_mac {
        // Distinct factory MAC per instance: EFUSE_RD_MAC_SPI_SYS_0
        // (0x6000_8844, low 32 bits = MAC[5..2]) + _1 (0x6000_8848, bits
        // [15:0] = MAC[1..0]). Two stations need distinct MACs to be
        // distinguishable on the shared air.
        let lo =
...
/// own real MAC; the medium is the only thing between them (the AP + the air).
/// They associate, get distinct DHCP leases (192.168.4.2 / .3), and any IP
/// traffic one sends to the other's address is routed by the AP. Stdout shows
/// both firmwares' logs interleaved — distinguish by the IP in each "GOT IP" line.
fn run_two_c3_wifi(
    args: &RunArgs,
