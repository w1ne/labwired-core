...
fn l32r_oracle() -> OracleCase {
    OracleCase::from_bytes(vec![
        ...
        0x06, 0x01, 0x00, // padding byte (literal must be 4-byte aligned at offset 4)
        0x00, // literal 0xCAFEBABE at offset 4 (4-byte LE)
        0xBE, 0xBA, 0xFE, 0xCA,
...
fn call0_oracle() -> OracleCase {
    OracleCase::from_bytes(vec![
        ...
        0x05, 0x00, 0x00, // padding byte at offset 3 (unreachable)
        0x00, // BREAK 1,15 at offset 4 (CALL0 jumps here; from_bytes also appends BREAK)
        0xF0, 0x41, 0x00,
    ])
    .expect(|st| {
...
fn call4_entry_retw_no_overflow() -> OracleCase {
    OracleCase::from_bytes(vec![
        ...
        0xF0, 0x41, 0x00, // IRAM_BASE+6: padding (unreachable)
        0x00, 0x00, // IRAM_BASE+8: ENTRY a1, 32  (w=0x004136)
        0x36, 0x41, 0x00, // IRAM_BASE+11: RETW  (w=0x000090)
        0x90, 0x00,
        0x00,
...
fn nested_2level_call_no_overflow() -> OracleCase {
    OracleCase::from_bytes(vec![
        ...
        0x15, 0x00, 0x00, // IRAM_BASE+3: padding
        0x00, // IRAM_BASE+4: ENTRY a1, 32
        0x36, 0x41, 0x00,
        // IRAM_BASE+7: CALL4 (imm18=1, target=IRAM_BASE+12)
        // base = ((IRAM_BASE+11)&~3) = IRAM_BASE+8; +4 → IRAM_BASE+12 ✓
        0x55, 0x00, 0x00, // IRAM_BASE+10: padding
        0x00, 0x00, // IRAM_BASE+12: ENTRY a1, 32
        0x36, 0x41, 0x00,
...
fn rotw_1() -> OracleCase {
    // ROTW 1: w=0x408010
    OracleCase::asm(".word 0x408010").expect(|st| {
