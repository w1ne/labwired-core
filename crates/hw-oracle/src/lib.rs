...
fn capture_sim_state(case: &OracleCase) -> OracleState {
    ...
    match &case.program {
        ...
        Program::Elf(path) => {
            ...
            for ph in &elf.program_headers {
                ...
                let offset_in_iram = vaddr.checked_sub(IRAM_BASE).unwrap_or_else(|| {
                    panic!(
    ...
    if let Some(mask) = init_state.init_interrupt {
        cpu.sr
            .set_raw(labwired_core::cpu::xtensa_sr::INTERRUPT, mask);
    }
    ...
    for _ in 0..MAX_STEPS {
        match cpu.step(&mut bus, &[], &sim_config) {
            Ok(()) => {}
...
fn capture_hw_state(case: &OracleCase) -> OracleState {
    ...
    let mut oc = OpenOcd::spawn_for(&board).expect("run_hw: failed to spawn OpenOCD");
        ...
        Program::Elf(path) => {
            ...
            for ph in &elf.program_headers {
                ...
                let words: Vec<u32> = padded
                    ...
                    .collect();
                oc.write_memory(vaddr, &words).unwrap_or_else(|e| {
                    panic!("run_hw: write_memory for ELF segment at 0x{vaddr:08X} failed: {e}")
                });
            }
            elf.entry as u32
        }
...
pub fn run_diff(case: OracleCase) {
    ...
    for (k, &v_sim) in &sim_state.regs {
        ...
        if v_sim != v_hw {
            mismatches.push(format!("reg {k}: sim 0x{v_sim:08X} vs hw 0x{v_hw:08X}"));
        }
    }
    if !mismatches.is_empty() {
        panic!("oracle diff failed:\n  {}", mismatches.join("\n  "));
    }
