...
fn test_real_world_iot_chain() {
    ...
    world.add_machine("controller".to_string(), Box::new(Machine::new(CortexM::new(), bus1)));
    ...
    world.add_machine("actuator".to_string(), Box::new(Machine::new(CortexM::new(), bus2)));
    ...
    world.add_machine("monitor".to_string(), Box::new(Machine::new(CortexM::new(), bus3)));

    // 3. Scenario Configuration
    
    // Set both radios to Channel 15
    world.machines.get_mut("actuator").unwrap().write_u8(radio_base + 0, 15).unwrap();
    world.machines.get_mut("monitor").unwrap().write_u8(radio_base + 0, 15).unwrap();
    ...
    assert_eq!(radio_status, 1, "Monitor should have received Wireless packet");
