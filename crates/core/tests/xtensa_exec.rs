...
fn test_exec_break_returns_breakpoint_error() {
    ...
    let err = cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).unwrap_err();
    match err {
...
fn test_exec_entry_rotates_window_base_by_callinc() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).unwrap();
...
fn step_ok(cpu: &mut XtensaLx7, bus: &mut SystemBus) {
    cpu.step(bus, &[], &labwired_core::SimulationConfig::default()).expect("step should succeed");
}
...
fn test_step_does_not_misdecode_s32i_n_as_l32e_outside_excm() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).expect("first step (s32i.n) should not error");
...
fn test_exec_movsp_safe_path() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).expect("MOVSP safe path must not error");
...
fn test_exec_movsp_overflow_raises_exception() {
    ...
    let err = cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).expect_err("MOVSP with live adjacent frame must raise exception");
    assert!(
...
fn test_exec_rotw_pos() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).expect("ROTW +1 must not error");
...
fn test_exec_rotw_neg_wraparound() {
    ...
    write_insns(&mut bus, TEST_PC as u64, &[enc_rotw(-1)]);
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).expect("ROTW -1 wraparound must not error");
...
fn test_exec_rotw_max_pos() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).expect("ROTW +7 must not error");
...
fn test_exec_rotw_max_neg() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).expect("ROTW -8 must not error");
...
fn test_exec_rotw_does_not_change_windowstart() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default()).expect("ROTW +3 must not error");
