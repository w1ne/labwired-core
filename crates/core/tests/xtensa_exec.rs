...
use labwired_core::cpu::xtensa_sr::{
    EPC1 as EPC1_ID, EPC2 as EPC2_ID, EPC3 as EPC3_ID, EPS2 as EPS2_ID, EPS3 as EPS3_ID,
    EXCCAUSE as EXCCAUSE_ID, SAR as SAR_ID, SCOMPARE1 as SCOMPARE1_ID, VECBASE as VECBASE_ID,
};
...
fn enc_sll(ar: u32, as_: u32) -> u32 {
    rrr(0xA, 0x1, ar, as_, 0)
}

/// Encode SRL ar, at  (op0=0, op1=0x1, op2=0x9, r=ar, s=0, t=at).
fn enc_srl(ar: u32, at: u32) -> u32 {
    rrr(0x9, 0x1, ar, 0, at)
}

/// Encode SRA ar, at  (op0=0, op1=0x1, op2=0xB, r=ar, s=0, t=at).
fn enc_sra(ar: u32, at: u32) -> u32 {
    rrr(0xB, 0x1, ar, 0, at)
}

/// Encode SRC ar, as_, at  (op0=0, op1=0x1, op2=0x8).
fn enc_src(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0x8, 0x1, ar, as_, at)
}
...
fn enc_ssl(as_: u32) -> u32 {
    rrr(0x4, 0x0, 0x1, as_, 0)
}

/// Encode SSR as_  (op0=0, op1=0, op2=0x4, r=0x0, s=as_, t=0).
fn enc_ssr(as_: u32) -> u32 {
    rrr(0x4, 0x0, 0x0, as_, 0)
}

/// Encode SSA8L as_  (op0=0, op1=0, op2=0x4, r=0x2, s=as_, t=0).
fn enc_ssa8l(as_: u32) -> u32 {
    rrr(0x4, 0x0, 0x2, as_, 0)
}

/// Encode SSA8B as_  (op0=0, op1=0, op2=0x4, r=0x3, s=as_, t=0).
fn enc_ssa8b(as_: u32) -> u32 {
    rrr(0x4, 0x0, 0x3, as_, 0)
}
...
fn enc_ssai(shamt: u32) -> u32 {
    rrr(0x4, 0x0, 0x4, shamt & 0xF, shamt >> 4)
}
...
fn write_insns(bus: &mut SystemBus, addr: u64, words: &[u32]) {
    for (i, &w) in words.iter().enumerate() {
        ...
        bus.write_u8(addr + off + 1, ((w >> 8) & 0xFF) as u8)
            .unwrap();
        bus.write_u8(addr + off + 2, ((w >> 16) & 0xFF) as u8)
            .unwrap();
    }
}
...
fn run_until_error(cpu: &mut XtensaLx7, bus: &mut SystemBus) -> SimulationError {
    for _ in 0..1000 {
        match cpu.step(bus, &[], &labwired_core::SimulationConfig::default()) {
            Ok(()) => {}
            Err(e) => return e,
        }
    }
...
fn test_exec_sub_wrapping() {
    ...
    write_insns(
        &mut bus,
        TEST_PC as u64,
        &[movi(2, 3), movi(3, 10), rrr(0xC, 0, 4, 2, 3), st0(4, 0, 0)],
    );
...
fn test_exec_nop_memw_advance_pc() {
    ...
    assert_eq!(
        cpu.get_pc(),
        TEST_PC + 6,
        "PC after NOP+MEMW should be TEST_PC+6"
    );
}
    ...
    match err {
        SimulationError::BreakpointHit(pc) => {
            assert_eq!(
                pc,
                TEST_PC + 9,
                "expected taken branch (PC = TEST_PC+9), got PC = {:#010x}",
                pc
            );
        }
        other => panic!("expected BreakpointHit, got {:?}", other),
    ...
    match err {
        SimulationError::BreakpointHit(pc) => {
            assert_eq!(
                pc,
                TEST_PC + 3,
                "expected not-taken branch (PC = TEST_PC+3), got PC = {:#010x}",
                pc
            );
        }
        other => panic!("expected BreakpointHit, got {:?}", other),
...
fn test_exec_j_forward() {
    ...
    assert!(
        matches!(err, SimulationError::BreakpointHit(_)),
        "expected BreakpointHit"
    );
    assert_eq!(
        cpu.get_register(2),
        42,
        "a2 should be 42 (MOVI at landing target)"
    );
}
...
fn test_exec_j_backward() {
    ...
    assert!(
        ...
        "expected BreakpointHit at TEST_PC, got {:?}",
        err
    );
}
...
fn test_exec_jx() {
    ...
    assert_eq!(
        cpu.get_register(2),
        42,
        "a2 should be 42 (JX landed at MOVI)"
    );
}
...
fn test_exec_call0_returns_via_ret() {
    ...
    assert_eq!(
        cpu.get_register(2),
        42,
        "a2 should be 42 (returned from subroutine)"
    );
    assert_eq!(
        cpu.get_register(0),
        TEST_PC + 3,
...
fn test_exec_callx8_jumps_and_writes_return_pc() {
    ...
    assert_eq!(
        cpu.get_register(8),
        expected_a8,
        "a8 should hold return PC with N=2 in bits[31:30]"
    );
...
fn test_exec_entry_rotates_window_base_by_callinc() {
    ...
    assert_eq!(
        cpu.regs.windowbase(),
        1,
        "WindowBase should advance by CALLINC=1"
    );
}
...
fn test_exec_entry_rotates_window_base_callinc2() {
    ...
    assert_eq!(
        cpu.regs.windowbase(),
        2,
        "WindowBase should advance by CALLINC=2"
    );
}
...
fn test_exec_entry_sets_windowstart_bit() {
    ...
    assert!(
        ...
        "WindowStart bit 1 should be set after ENTRY with CALLINC=1"
    );
}
...
fn test_exec_retw_rotates_window_back() {
    ...
    assert_eq!(
        cpu.regs.windowbase(),
        0,
        "RETW should restore WindowBase to 0 (1 - N=1)"
    );
}
...
fn test_exec_retw_clears_windowstart_bit() {
    ...
    assert!(
        cpu.regs.windowstart_bit(1),
        "pre-condition: WS[1] should be set"
    );
    ...
    assert!(
        !cpu.regs.windowstart_bit(1),
        "RETW should clear WindowStart bit 1 (old WB)"
    );
}
...
fn test_exec_call4_entry_retw_full_cycle() {
    ...
    assert!(
        matches!(err, SimulationError::BreakpointHit(_)),
        "expected BreakpointHit at return, got {:?}",
        err
    );
    ...
    assert_eq!(
        cpu.regs.windowbase(),
        0,
        "WB should be restored to 0 after RETW"
    );
    // WindowStart[1] should be cleared
    assert!(
        !cpu.regs.windowstart_bit(1),
        "WS[1] should be cleared by RETW"
    );
}
...
fn enc_mull(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0x8, 0x2, ar, as_, at)
}
fn enc_muluh(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0xA, 0x2, ar, as_, at)
}
fn enc_mulsh(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0xB, 0x2, ar, as_, at)
}
fn enc_mul16u(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0xC, 0x1, ar, as_, at)
}
fn enc_mul16s(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0xD, 0x1, ar, as_, at)
}
...
fn test_exec_mull_basic() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0x2468ACF0,
        "mull: 0x12345678 * 2 should give 0x2468ACF0"
    );
}
...
fn test_exec_mull_overflow_wraps() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0x00000001,
        "mull: 0xFFFFFFFF * 0xFFFFFFFF low32 should wrap to 0x1"
    );
}
...
fn test_exec_muluh_basic() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0x40000000,
        "muluh: 0x80000000*0x80000000 upper32 should be 0x40000000"
    );
}
...
fn test_exec_mulsh_negative() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0x10000000,
        "mulsh: (-2^30)*(-2^30) upper32 should be 0x10000000"
    );
}
...
fn test_exec_mulsh_mixed() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0xF0000000,
        "mulsh: (-2^30)*(2^30) upper32 should be 0xF0000000"
    );
}
...
fn test_exec_mul16u_zero_extend() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0x06260060,
        "mul16u: 0x1234*0x5678 = 0x06260060; high bits ignored"
    );
}
...
fn test_exec_mul16s_sign_extend() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0xFFFF0000,
        "mul16s: 0x8000 (=-32768) * 2 = 0xFFFF0000"
    );
}
...
fn enc_quos(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0xD, 0x2, ar, as_, at)
}
fn enc_quou(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0xC, 0x2, ar, as_, at)
}
fn enc_rems(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0xF, 0x2, ar, as_, at)
}
fn enc_remu(ar: u32, as_: u32, at: u32) -> u32 {
    rrr(0xE, 0x2, ar, as_, at)
}
...
fn test_exec_quos_min_div_neg_one() {
    ...
    assert_eq!(
        cpu.get_register(3) as i32,
        i32::MIN,
        "quos: i32::MIN / -1 should wrap to i32::MIN (no exception)"
    );
}
...
fn test_exec_quou_large_unsigned() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0xF,
        "quou: 0xF0000000 / 0x10000000 should be 0xF"
    );
}
...
fn test_exec_rems_negative_dividend() {
    ...
    assert_eq!(
        cpu.get_register(3) as i32,
        -2,
        "rems: -100 % 7 should be -2 (sign of dividend)"
    );
}
...
fn test_exec_quos_div_by_zero_raises_exception() {
    ...
    assert!(
        ...
        "quos div-by-zero should return ExceptionRaised{{cause:6}}, got: {:?}",
        err
    );
    assert_eq!(
        cpu.sr.read(EXCCAUSE_ID),
        6,
        "EXCCAUSE SR should be 6 after div-by-zero"
    );
}
...
fn test_exec_quou_div_by_zero() {
    ...
    assert!(
        matches!(err, SimulationError::ExceptionRaised { cause: 6, .. }),
        "quou div-by-zero should return ExceptionRaised{{cause:6}}, got: {:?}",
        err
    );
    assert_eq!(
        cpu.sr.read(EXCCAUSE_ID),
...
fn test_exec_rems_div_by_zero() {
    ...
    assert!(
        matches!(err, SimulationError::ExceptionRaised { cause: 6, .. }),
        "rems div-by-zero should return ExceptionRaised{{cause:6}}, got: {:?}",
        err
    );
    assert_eq!(
        cpu.sr.read(EXCCAUSE_ID),
...
fn test_exec_remu_div_by_zero() {
    ...
    assert!(
        matches!(err, SimulationError::ExceptionRaised { cause: 6, .. }),
        "remu div-by-zero should return ExceptionRaised{{cause:6}}, got: {:?}",
        err
    );
    assert_eq!(
        cpu.sr.read(EXCCAUSE_ID),
...
fn enc_min(ar: u32, as_: u32, at: u32) -> u32 {
    enc_lsci_rrr(4, ar, as_, at)
}
fn enc_max(ar: u32, as_: u32, at: u32) -> u32 {
    enc_lsci_rrr(5, ar, as_, at)
}
fn enc_minu(ar: u32, as_: u32, at: u32) -> u32 {
    enc_lsci_rrr(6, ar, as_, at)
}
fn enc_maxu(ar: u32, as_: u32, at: u32) -> u32 {
    enc_lsci_rrr(7, ar, as_, at)
}
...
fn step_ok(cpu: &mut XtensaLx7, bus: &mut SystemBus) {
    cpu.step(bus, &[], &labwired_core::SimulationConfig::default())
...
fn test_exec_min_negative() {
    ...
    cpu.reset(&mut bus).unwrap();
    cpu.set_pc(TEST_PC);
    cpu.set_register(4, (-1i32) as u32);
...
fn test_exec_maxu() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0xFFFFFFFF,
        "MAXU(0xFFFFFFFF,1) should be 0xFFFFFFFF"
    );
}
...
fn test_exec_sext_t0_negative() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0xFFFFFF80,
        "SEXT(0x80, sa=7) should be 0xFFFFFF80"
    );
}
...
fn test_exec_sext_t0_positive() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0x0000007F,
        "SEXT(0x7F, sa=7) should be 0x0000007F"
    );
}
...
fn test_exec_clamps_t0_overflow_pos() {
    ...
    assert_eq!(
        cpu.get_register(3),
        127,
        "CLAMPS(200, sa=7) should saturate to 127"
    );
}
...
fn test_exec_s32c1i_uncontended_success() {
    ...
    assert_eq!(
        ...
        0xCAFEBABE,
        "SCOMPARE1 must be set before test"
    );
...
fn test_exec_l32ai_basic() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0xDEAD_C0DE,
        "L32AI should load 0xDEAD_C0DE into a3"
    );
}
...
fn test_exec_l32ai_imm_offset() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0x1234_5678,
        "L32AI imm=4 should load from DATA_ADDR+4"
    );
}
...
fn test_exec_entry_no_overflow_when_target_frame_clear() {
    ...
    assert_eq!(
        cpu.regs.windowbase(),
        1,
        "No-OF: WindowBase should rotate to 1"
    );
    assert!(
        cpu.regs.windowstart_bit(1),
        "No-OF: WindowStart[1] should be set"
    );
    assert_eq!(cpu.ps.callinc(), 0, "No-OF: CALLINC should be cleared");
    assert_eq!(cpu.pc, TEST_PC + 3, "No-OF: PC should advance past ENTRY");
...
fn test_exec_retw_no_underflow_when_destination_frame_set() {
    ...
    assert_eq!(
        cpu.pc, expected_pc,
        "No-UF: PC should be set to return address"
    );
    // WB rotated back to 0
    assert_eq!(
        cpu.regs.windowbase(),
        0,
        "No-UF: WindowBase should rotate to 0"
    );
    // WS[1] cleared by normal RETW
    assert!(
        !cpu.regs.windowstart_bit(1),
        "No-UF: WindowStart[1] should be cleared"
    );
    // EPC1 must NOT have been set (no exception)
    assert_eq!(
        cpu.sr.read(EPC1_ID),
    ...
    assert!(
        !cpu.ps.excm(),
        "No-UF: PS.EXCM should remain clear (no UF exception)"
    );
}
...
fn test_exec_l32e_in_excm_reads() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0xCAFE_BABE,
        "L32E should load from EA into at"
    );
...
fn test_exec_s32e_negative_offset() {
    ...
    assert_eq!(
        stored, 0xABCD_1234,
        "S32E with imm=-64 should write to as_-64"
    );
}
...
fn test_exec_s32e_hw_oracle_bytes() {
    ...
    assert_eq!(
        stored, 0x1111_2222,
        "HW-oracle s32e a3,a4,-16 should store a3 at a4-16"
    );
}
...
fn test_exec_l32e_hw_oracle_bytes() {
    ...
    assert_eq!(
        cpu.get_register(3),
        0xFEED_FACE,
        "HW-oracle l32e a3,a4,-16 should load into a3"
    );
}
...
fn test_exec_s32e_a0_a1_negative_12_real_firmware() {
    ...
    assert_eq!(
        stored, 0xC0FFEE00,
        "real-firmware s32e a0,a1,-12 should store a0 at a1-12"
    );
...
fn test_step_does_not_misdecode_s32i_n_as_l32e_outside_excm() {
    ...
    assert!(
        cpu.ps.excm(),
        "reset leaves PS.EXCM=1, we will clear it below"
    );
    cpu.ps.set_excm(false);
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default())
        .expect("first step (s32i.n) should not error");
...
fn test_exec_movsp_safe_path() {
    ...
    assert!(
        !cpu.regs.windowstart_bit(1),
        "pre: WS[1] must be clear for safe path"
    );
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default())
        .expect("MOVSP safe path must not error");

    // a[3] = a[4] = 0xDEAD_BEEF
    assert_eq!(
        cpu.get_register(3),
        0xDEAD_BEEF,
        "MOVSP: a3 must equal a4 after safe-path move"
    );
    // a[4] (source) unchanged
    assert_eq!(
        cpu.get_register(4),
        0xDEAD_BEEF,
        "MOVSP: a4 (source) must be unchanged"
    );
    // PC advanced by 3
    ...
    assert_eq!(
        cpu.regs.windowbase(),
        0,
        "MOVSP: WindowBase must not change"
    );
    assert_eq!(
        cpu.regs.windowstart(),
        0x1,
        "MOVSP: WindowStart must not change"
    );
}
...
fn test_exec_rotw_pos() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default())
        .expect("ROTW +1 must not error");

    assert_eq!(
        cpu.regs.windowbase(),
        4,
        "ROTW +1: WB must be (3+1)&0xF = 4"
    );
    assert_eq!(
        cpu.regs.windowstart(),
        ws_before,
        "ROTW +1: WindowStart must not change"
    );
...
fn test_exec_rotw_neg_wraparound() {
    ...
    write_insns(&mut bus, TEST_PC as u64, &[enc_rotw(-1)]);
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default())
        .expect("ROTW -1 wraparound must not error");

    assert_eq!(
        cpu.regs.windowbase(),
        15,
        "ROTW -1 from WB=0 must wrap to 15"
    );
    assert_eq!(
        cpu.regs.windowstart(),
        ws_before,
        "ROTW wraparound: WindowStart must not change"
    );
}
...
fn test_exec_rotw_max_pos() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default())
        .expect("ROTW +7 must not error");

    assert_eq!(
        cpu.regs.windowbase(),
        9,
        "ROTW +7: WB must be (2+7)&0xF = 9"
    );
    assert_eq!(
        cpu.regs.windowstart(),
        ws_before,
        "ROTW max-pos: WindowStart must not change"
    );
}
...
fn test_exec_rotw_max_neg() {
    ...
    cpu.step(&mut bus, &[], &labwired_core::SimulationConfig::default())
        .expect("ROTW -8 must not error");

    assert_eq!(
        cpu.regs.windowbase(),
        13,
        "ROTW -8: WB must be (5-8+16)&0xF = 13"
    );
    assert_eq!(
        cpu.regs.windowstart(),
        ws_before,
        "ROTW max-neg: WindowStart must not change"
    );
}
...
fn test_general_exception_div_by_zero_redirects_pc() {
    ...
    assert!(
        ...
        "div-by-zero: ExceptionRaised{{cause:6, pc=TEST_PC}} expected, got: {:?}",
        err
    );
    ...
    assert!(
        cpu.ps.excm(),
        "PS.EXCM must be 1 after general exception entry"
    );
    ...
    assert_eq!(
        old_ps_raw & 0x10,
        0,
        "pre: PS.EXCM should have been 0 before exception"
    );
}
...
fn test_general_exception_remu_redirects_pc() {
    ...
    assert!(
        matches!(err, SimulationError::ExceptionRaised { cause: 6, pc } if pc == TEST_PC),
        "remu div-by-zero: ExceptionRaised{{cause:6}} expected, got {:?}",
        err
    );
    assert_eq!(
        cpu.sr.read(EPC1_ID),
        TEST_PC,
        "EPC1 must be set to faulting PC"
    );
    assert_eq!(cpu.sr.read(EXCCAUSE_ID), 6, "EXCCAUSE must be 6");
...
fn test_general_exception_movsp_alloca_redirects_pc() {
    ...
    assert_eq!(
        cpu.sr.read(EPC1_ID),
        TEST_PC,
        "EPC1 must be set to faulting MOVSP PC"
    );
    assert_eq!(
        cpu.sr.read(EXCCAUSE_ID),
        5,
    ...
    assert!(
        cpu.ps.excm(),
        "PS.EXCM must be 1 after MOVSP alloca exception"
    );
    let vecbase = cpu.sr.read(VECBASE_ID);
    assert_eq!(
        cpu.get_pc(),
...
fn test_general_exception_does_not_advance_pc_into_epc1() {
    ...
    assert_ne!(
        cpu.get_pc(),
        faulting_pc + 3,
        "PC must not be advanced to faulting_pc+3"
    );
    let vecbase = cpu.sr.read(VECBASE_ID);
    assert_eq!(
        cpu.get_pc(),
...
fn test_exec_rfe_clears_excm_and_jumps() {
    ...
    assert_eq!(
        cpu.ps.intlevel(),
        3,
        "RFE: PS.INTLEVEL must be left unchanged"
    );
}
...
fn test_exec_rfe_round_trip_via_epc1() {
    ...
    assert_eq!(
        cpu.get_pc(),
        return_addr,
        "RFE round-trip: PC must be exactly EPC1"
    );
...
fn test_exec_rfi_2() {
    ...
    assert_eq!(
        cpu.ps.as_raw(),
        saved_ps,
        "RFI 2: PS must be fully restored from EPS2"
    );
...
fn test_exec_rfi_3() {
    ...
    assert_eq!(
        cpu.ps.as_raw(),
        saved_ps,
        "RFI 3: PS must be fully restored from EPS3"
    );
...
fn test_exec_break_1_15_halts_with_pc() {
    ...
    write_insns(&mut bus, TEST_PC as u64, &[enc_break(1, 15)]);
