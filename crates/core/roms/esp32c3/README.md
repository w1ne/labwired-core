# Vendored ESP32-C3 boot-ROM images

Flat mask-ROM images for the faithful `--rom-boot` path, embedded into
non-wasm builds so LabWired runs unmodified ESP32-C3 firmware out of the box
with no ESP toolchain installed.

| file | window | size |
|---|---|---|
| `esp32c3_rom.bin` | IROM (instruction bus) `0x4000_0000..0x4006_0000` | 384 KiB |
| `esp32c3_drom.bin` | DROM (data bus) `0x3FF0_0000..0x3FF2_0000` | 128 KiB |

## Provenance

Extracted from Espressif's published `esp32c3_rev3_rom.elf`
...
`scripts/make_esp32c3_rom_bins.py`. The Rust extractor
(`crates/core/src/boot/esp32c3_rom.rs::extract_rom_images`) produces
byte-identical output from the same ELF.

The ROM contents are the ESP32-C3 mask ROM, copyright Espressif Systems
