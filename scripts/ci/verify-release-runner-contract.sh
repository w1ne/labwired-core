...
require_literal "$action" 'default: "v0.25.0"' 'core action defaults to the supported public release'
action_inputs=$(awk '
...
safe_action_sha=64ed5d9723e2d9f5f4a851a81a3468025671d94e
safe_action_version=v0.23.0
...
for doc in docs/ci_integration.md docs/ci_test_runner.md docs/integration-templates/github-actions.yml docs/integration-templates/gitlab-ci.yml docs/integration-templates/README.md docs/reference_client_flows.md .github/actions/labwired-test/README.md; do
  ...
  require_absent_literal "$doc" 'v0.22.2' "$doc does not retain the superseded Core release version"
done
...
if ! git cat-file -e "${safe_action_sha}^{commit}" 2>/dev/null; then
  ...
  if [[ -n "$pinned_action" ]]; then
    require_block_literal "$pinned_action" 'default: "v0.23.0"' 'pinned action defaults to the supported public release'
  ...
  if [[ -n "$pinned_action_readme" ]]; then
    require_block_literal "$pinned_action_readme" 'defaults to `v0.23.0`' 'pinned action README states the supported public release'
...
require_literal Cargo.toml 'version = "0.25.0"' 'workspace metadata uses the current release version'
require_literal CHANGELOG.md '## [0.25.0] - 2026-09-22' 'changelog records the current release version'
require_literal README.md 'LABWIRED_VERSION=v0.25.0' 'public README pins the current release version'
