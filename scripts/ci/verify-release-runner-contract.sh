...
require_literal "$workflow" 'archive_format: zip' 'Windows archive matrix uses ZIP packaging'
require_literal "$workflow" 'binary: labwired.exe' 'Windows archive matrix uses the native CLI name'
...
require_literal "$workflow" "Copy-Item \"target/\${{ matrix.target }}/release/\${{ matrix.binary }}\" 'dist/labwired.exe'" 'Windows archive package copies the CLI'
require_literal "$workflow" "Copy-Item \"target/\${{ matrix.target }}/release/labwired-dap.exe\" 'dist/labwired-dap.exe'" 'Windows archive package copies the debug adapter'
require_literal "$workflow" "Compress-Archive -Path 'dist/labwired.exe', 'dist/labwired-dap.exe'" 'Windows archive contains both CLI and debug adapter'
...
require_block_literal "$release_block" 'find . -maxdepth 1 -type f' 'release generates checksums from release assets'
require_block_literal "$release_block" 'SHA256SUMS' 'release publishes an archive checksum manifest'
require_block_literal "$release_block" 'files: dist/*' 'release uploads every generated archive asset and checksum manifest'
