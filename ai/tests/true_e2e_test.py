...
class TrueE2ETest:
    ...
    def run(self):
        ...
        try:
            ...
            system_path = self.step5_generate_system_config(ir_path)
            ...
            self.step8_behavioral_verification(system_path)
            ...
            print(f"  ✓ Test Firmware Src: {firmware_path}")
            print()
            print("🚀 READY FOR FIRMWARE BUILD + SIMULATION!")
            print("   1) Compile test_firmware.rs into an ELF for your target")
            print(f"   2) Run: labwired --system {system_path} --firmware <compiled_firmware.elf>")
            print()
    ...
    def step1_full_ai_ingestion(self):
        ...
        # Check if we can skip ingestion (for rapid iteration)
        if output_yaml.exists() and output_yaml.stat().st_size > 0:
            print(f"  ✓ Skipping ingestion (found existing {output_yaml.name})", flush=True)
            return output_yaml

        # Run the FULL AI ingestion pipeline
        print("  → Running multi-stage AI ingestion (in-process)...", flush=True)

        # Import main dynamically to avoid top-level side effects if any
        sys.path.append(str(Path(__file__).parent.parent))
        from labwired_ai.main import main as ai_main

        args = [
            "ingest-datasheet",
            "--pdf", str(self.datasheet_path.absolute()),
            "--pages", self.pages,
            "--name", self.device_name,
            "--output", str(output_yaml.absolute())
        ]

        print(f"  → Invoking labwired_ai.main with: {args}")
        ...
        try:
            ...
            if e.code != 0:
                raise RuntimeError(f"AI Main exited with code {e.code}")
    ...
    def step5_generate_system_config(self, ir_path):
        """Generate system.yaml configuration using CLI"""
        ...
        # 2. asset add-peripheral
        subprocess.run([
            "cargo", "run", "--quiet",
            "--manifest-path", str(Path(__file__).parent.parent.parent / "core" / "crates" / "cli" / "Cargo.toml"),
            "--", "asset", "add-peripheral",
            "--chip", str(project_dir / "stm32f401.yaml"),
            "--id", self.device_name,
            "--base", "0x40001000",
            "--ir-path", str(ir_path)
        ], check=True)
        ...
        print(f"  ✓ Initialized project in {project_dir}")
        print(f"  ✓ Added peripheral {self.device_name} via CLI")
        print()
        return system_path

    def step8_behavioral_verification(self, system_path):
        """Verify that the generated IR contains the expected behavioral logic"""
        print("Step 8: Behavioral Verification...")

        ir_path = self.test_dir / f"{self.device_name.lower()}_ir.json"
        with open(ir_path) as f:
            ir_data = json.load(f)
            periph = list(ir_data['peripherals'].values())[0]

            timing = periph.get('timing', [])
            print(f"  ✓ Found {len(timing)} behavioral timing hooks")

            if len(timing) == 0:
                print("  ⚠️  No behaviors extracted. Digital Twin is purely static.")
            else:
                for t in timing:
                    print(f"  ✓ Behavior: {t['id']}")
