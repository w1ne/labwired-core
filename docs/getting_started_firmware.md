...
> **The firmware boundary.** LabWired does **not** ship a firmware HAL. Build firmware against the vendor SDK you would use on real silicon — ST CMSIS / STM32CubeMX HAL, Nordic nrfx, Raspberry Pi Pico SDK, ESP-IDF, or hand-rolled register writes. The simulator models the peripheral/register side; firmware sees the same memory-mapped registers it would on hardware. There is no `labwired.h` to include and no Labwired-provided abstractions to link against.
>
> Per-board coverage tables under [`docs/boards/`](boards/) list exactly which peripherals are modeled for each chip. If your firmware touches a register that isn't modeled yet, you'll get a `MemoryAccessViolation` or a stuck polling loop — see [Common Runtime Issues](#4-common-runtime-issues) below.
