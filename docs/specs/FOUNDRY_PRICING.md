...
> **Asset Generation**: Synthesizing a *new* peripheral or MCU from a prompt/datasheet has a **dynamic cost** based on complexity (LLM token usage, architectural novelty, and required verification passes). A simple peripheral might cost 10-50 runs, whereas onboarding a completely new MCU architecture could cost thousands of runs. The API provides an `/estimate` endpoint to quote the cost before execution.
...
| Feature | Builder | Team | Enterprise |
| :--- | :---: | :---: | :---: |
