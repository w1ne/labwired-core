...
| 8 | Cortex-M LSL/LSR/ASR immediate now set the carry flag (was N/Z only) | PR #191 | **Caught BY silicon, not pending it.** Adding xpsr/NZCV diffing to `thumb_oracles` failed `lsrs_immediate` against the bench F103 (sim C=0 vs silicon C=1). Fixed and re-verified — 38/38 hw+diff on the F103. | STM32F103 (bench) | ✅ **silicon-verified 2026-06-08** |
| 9 | F103 WDT (IWDG/WWDG) + USART2 register reset values | PR #192 | Reset values probed off the bench F103 via OpenOCD, pinned as `stm32f1_mmio_diff` RESET_CASES; sim matches silicon. | STM32F103 (bench) | ✅ **silicon-verified 2026-06-08** |
