...
- **No Zephyr boot.** The nRF5340 profile is validated against unmodified
  upstream Zephyr; this one is not. Zephyr's kernel tick on nRF54L is driven by
  the GRTC, so a Zephyr boot needs the GRTC model to satisfy a SYSCOUNTER poll.
- **No silicon diff.** Nothing here has been compared against a real nRF54L15
  ...
  we have run*, not *the model matches silicon*.
- **The radio is not modelled at all** — no BLE, no 802.15.4.
...
| RADIO (BLE / 802.15.4)| 0x5008_A000               | ❌ not mapped  | Not modelled                                                  |
