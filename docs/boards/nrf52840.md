...
| [`nrf52_conformance`](../../crates/hw-oracle/tests/nrf52_conformance.rs) | behavioral digest (firmware on sim + silicon) | **13/16** (3 documented residuals) |
...
## Known residuals (behavioral conformance, 13/16)

These three behavioral fields diverge sim-vs-silicon and are documented in
[`nrf52_conformance.rs`](../../crates/hw-oracle/tests/nrf52_conformance.rs)
(baseline ratcheted at 13 — raise it when any is closed):

- **ECB (AES-128):** silicon computes the FIPS-197 ciphertext; the sim ECB
  model does not implement AES (returns 0). Needs an AES core.
- **TEMP:** silicon fires DATARDY with an in-range reading; the sim TEMP model
  never raises DATARDY. Needs a measurement model.
- **GPIOTE task→GPIO:** the sim drives a pin via the GPIOTE SET task where
  silicon leaves it low under these conditions — the sim is over-permissive on
  the task/event routing. (The plain GPIO register interface is separately
  strict-clean; this is the GPIOTE peripheral, not GPIO read/write.)
