...
```mermaid
graph TD
    A["AI Agent / CI Bot"] -->|"POST /v1/twins/simulate (API Key)"| B[FastAPI Backend]
    B --> C[Job Queue]
    C --> D["LabWired Core (Rust)\nFormal Simulation Engine\n(Docker sandbox)"]
    D --> E[Artifact Storage\nLocal filesystem]
    E -->|"VCD + result.json + IR"| B
    B -->|"run_id + status (async polling)"| A

    F["Human Dev (Optional)"] -->|"Browser"| G[React Dashboard]
    G -->|"catalog, usage, downloads"| B
```

**Infrastructure (Hetzner VPS CX21, €7/mo)**:
| Component | MVP | v1.1+ |
| :--- | :--- | :--- |
| API Server | FastAPI (Python) | same |
| Job Queue | `asyncio.Queue` (in-process) | Redis |
| Database | SQLite | PostgreSQL |
| Artifact Storage | Local filesystem under `/srv/artifacts/` | S3-compatible (Hetzner Object Storage) |
| Simulation Sandbox | Docker (namespaced, resource-limited) | same |
| Reverse Proxy | Caddy (auto TLS) | same |
| Frontend | React + Vite (served by Caddy) | same |
...
- Per-key rate limits enforced at FastAPI middleware layer (before any business logic runs).
- Keys can be rotated or revoked instantly. Old keys become invalid within seconds.
