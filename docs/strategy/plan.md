...
- [ ] Build C firmware: LED blinky on STM32F103
- [ ] Verify GPIO peripheral writes work
- [ ] Run firmware in `labwired-cli` successfully
- [ ] Document GPIO usage in examples

**Success Metric**: LED state changes visible in logs

### Milestone 2: "Sensor Connected"
**Deliverable**: I2C temperature sensor integrated

- [ ] Create YAML descriptor for TMP102 I2C sensor
- [ ] Implement sensor read handlers (mock 25°C data)
- [ ] Update `system.yaml` to wire sensor to I2C1
- [ ] Build firmware that reads sensor + toggles LED
- [ ] Verify sensor data appears in logs

**Success Metric**: Firmware reads temperature value from I2C

### Milestone 3: "VS Code Debugs It"
**Deliverable**: Basic debugging via DAP

> **Strategy**: Use **Cortex-Debug** and **Peripheral Viewer** as reference implementations. Both are MIT-licensed and provide proven patterns for peripheral viewers, memory inspectors, and SVD support.

**Week 1 (v0.1.0) - Basic Debugging**:
- [ ] Verify DAP server launches firmware
- [ ] Verify breakpoints work
- [ ] Verify step-through works
- [ ] Verify CPU register inspection works
- [ ] Create `.vscode/launch.json` template for demo
- [ ] Document debugging workflow in README

**Week 2-3 (v0.2.0) - Hardware Features** (Post-Release):
- [ ] Clone reference repos: `Marus/cortex-debug`, `mcu-debug/peripheral-viewer`
- [ ] Study SVD parsing from Peripheral Viewer
- [ ] Implement peripheral register tree view
- [ ] Implement memory inspector (hex/ASCII)
- [ ] Add SVD file support for STM32

**Success Metric**: User can set breakpoint, step through code, and see register values

**Reference Implementations**:
- **Cortex-Debug**: `github.com/Marus/cortex-debug` (memory viewer, DAP integration)
- **Peripheral Viewer**: `github.com/mcu-debug/peripheral-viewer` (SVD parsing, register tree)

### Milestone 4: "Case Study Published"
**Deliverable**: Professional demo materials

- [ ] Write 1-page case study: "Debugging STM32 Without Hardware"
  - Problem: Hardware delays, limited availability
  - Solution: LabWired simulation
  - Results: Metrics (time saved, iteration speed)
- [ ] Record demo GIF (30-60s): Launch → Breakpoint → Step → Inspect
- [ ] Update `labwired-core/README.md` with demo section
- [ ] Add case study to `docs/examples/`

**Success Metric**: Compelling story for HN/Reddit announcement

### Milestone 5: "Public Release"
**Deliverable**: v0.1.0 on GitHub

- [ ] Fresh clone test on clean system
- [ ] Follow README from scratch
- [ ] Verify demo works end-to-end
- [ ] Create GitHub release (v0.1.0)
- [ ] Write release notes
- [ ] Draft announcement (HN, Reddit, Twitter)
...
**Rationale**: By providing a high-end debugging experience natively in VS Code, LabWired becomes the preferred tool for embedded developers, eliminating the need for expensive commercial debuggers for simulation workflows. See [DEBUGGING_STRATEGY.md](../../core/docs/DEBUGGING_STRATEGY.md) for technical details.
